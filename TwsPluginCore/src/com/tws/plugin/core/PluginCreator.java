...
public class PluginCreator {
	 ...
	 * @return
	 */
	public static DexClassLoader createPluginClassLoader(String absolutePluginApkPath, boolean isStandalone, String[] dependences,
		...
		if (!isStandalone) {// 非独立插件
			return new PluginClassLoader(absolutePluginApkPath, optDir.getAbsolutePath(), libDir.getAbsolutePath(), PluginLoader.class.getClassLoader(),// 宿主classloader
					dependences,// 插件依赖的插件
					null);
		} else {// 独立插件
	...
	public static Resources createPluginResource(String mainApkPath, Resources mainRes, PluginDescriptor pluginDescriptor) {
		...
		if (new File(absolutePluginApkPath).exists()) {
			...
			try {
				...
				RefInvoker.invokeMethod(assetMgr, AssetManager.class.getName(), "addAssetPaths", new Class[] { String[].class }, new Object[] { assetPaths });
	 ...
	 * @return
	 */
	public static Context createPluginContext(PluginDescriptor pluginDescriptor, Context base, Resources pluginRes, DexClassLoader pluginClassLoader) {
