...
public class PluginInjector {
	...
	static void injectBaseContext(Context context) {
		...
		Context base = (Context) RefInvoker.getFieldObject(context, ContextWrapper.class.getName(), android_content_ContextWrapper_mBase);
		Context newBase = new PluginBaseContextWrapper(base);
		RefInvoker.setFieldObject(context, ContextWrapper.class.getName(), android_content_ContextWrapper_mBase, newBase);
	}
	...
	static void injectInstrumetionFor360Safe(Activity activity, Instrumentation pluginInstrumentation) {
		...
		// 所以这里再做一次检查
		Instrumentation instrumention = (Instrumentation) RefInvoker.getFieldObject(activity, Activity.class.getName(), android_app_Activity_mInstrumentation);
	...
	static void resetActivityContext(final Context pluginContext, final Activity activity, final int pluginAppTheme) {
		...
		if (Build.VERSION.SDK_INT >= 11) {
			RefInvoker.invokeMethod(activity.getWindow().getLayoutInflater(), LayoutInflater.class.getName(), "setPrivateFactory",
					new Class[] { LayoutInflater.Factory2.class }, new Object[] { activity });
		}
	}

	static void resetWindowConfig(final Context pluginContext, final PluginDescriptor pd, final Activity activity, final ActivityInfo activityInfo,
	...
	/* package */static void replaceReceiverContext(Context baseContext, Context newBase) {
		...
		if (baseContext.getClass().getName().equals("android.app.ContextImpl")) {
			ContextWrapper receiverRestrictedContext = (ContextWrapper) RefInvoker.invokeMethod(baseContext, "android.app.ContextImpl",
					"getReceiverRestrictedContext", (Class[]) null, (Object[]) null);
	...
	/* package */static void replacePluginServiceContext(String servieName, Service service) {
		...
		RefInvoker.setFieldObject(service, ContextWrapper.class.getName(), "mBase",
				PluginLoader.getNewPluginComponentContext(plugin.pluginContext, service.getBaseContext(), pd.getApplicationTheme()));
	...
	public static void hackHostClassLoaderIfNeeded() {
		Object mLoadedApk = RefInvoker.getFieldObject(PluginLoader.getApplication(), Application.class.getName(), "mLoadedApk");
		if (mLoadedApk == null) {
			// 重试一次
			mLoadedApk = RefInvoker.getFieldObject(PluginLoader.getApplication(), Application.class.getName(), "mLoadedApk");
		}
		if (mLoadedApk == null) {
			// 换个方式再试一次
		...
		if (mLoadedApk != null) {
			ClassLoader originalLoader = (ClassLoader) RefInvoker.getFieldObject(mLoadedApk, "android.app.LoadedApk", "mClassLoader");
			if (!(originalLoader instanceof HostClassLoader)) {
				HostClassLoader newLoader = new HostClassLoader("", PluginLoader.getApplication().getCacheDir().getAbsolutePath(), PluginLoader
						.getApplication().getCacheDir().getAbsolutePath(), originalLoader);
