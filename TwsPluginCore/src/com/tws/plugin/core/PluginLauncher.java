...
public class PluginLauncher implements Serializable {
	...
	public LoadedPlugin startPlugin(PluginDescriptor pluginDescriptor) {
		...
		if (plugin == null) {
			...
			Resources pluginRes = PluginCreator.createPluginResource(PluginLoader.getApplication().getApplicationInfo().sourceDir, PluginLoader
					.getApplication().getResources(), pluginDescriptor);
			...
			Context pluginContext = PluginCreator.createPluginContext(pluginDescriptor, PluginLoader.getApplication().getBaseContext(), pluginRes,
					pluginClassLoader);
			...
			TwsLog.e(TAG, "startPlugin loadedPluginMap.put:" + pluginDescriptor.getPackageName() + " plugin=" + plugin + " loadedPluginMap.size="
					+ loadedPluginMap.size());
			...
			try {
				ActivityThread.installPackageInfo(PluginLoader.getApplication(), pluginDescriptor.getPackageName(), pluginDescriptor, pluginClassLoader,
						pluginRes, pluginApplication);
	...
	private Application callPluginApplicationOnCreate(Context pluginContext, DexClassLoader classLoader, PluginDescriptor pluginDescriptor) {
		...
		try {
			...
			application = Instrumentation.newApplication(classLoader.loadClass(pluginDescriptor.getApplicationName()), pluginContext);
	...
	public void stopPlugin(String packageName, PluginDescriptor pluginDescriptor) {
		...
		if (mInstance != null) {
			HashMap<BroadcastReceiver, ArrayList<IntentFilter>> mReceivers = (HashMap<BroadcastReceiver, ArrayList<IntentFilter>>) RefInvoker.getFieldObject(
					mInstance, "android.support.v4.content.LocalBroadcastManager", "mReceivers");
			if (mReceivers != null) {
				...
				while (ir.hasNext()) {
					...
					if (item.getClass().getClassLoader() == plugin.pluginClassLoader) {
						RefInvoker.invokeMethod(mInstance, "android.support.v4.content.LocalBroadcastManager", "unregisterReceiver",
